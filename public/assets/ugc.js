...
//
// 置き場所に注意: public/actress/ はビルドのたびに作り直され、.gitignore の
// 対象でもある。そこに置くと commit されず、ビルドで消える。ここに置くこと。
...
(function () {
  ...
  function voterHash() {
    const saved = localStorage.getItem('darekore-voter')
    if (saved) return saved

    const bytes = crypto.getRandomValues(new Uint8Array(16))
    const made = Array.from(bytes, (b) => b.toString(16).padStart(2, '0')).join('')

    localStorage.setItem('darekore-voter', made)
    return made
  }
  ...
  const voted = () => localStorage.getItem(`darekore-voted-${slug}`) === '1'

  async function loadVotes() {
    const url = `${api}/rest/v1/performer_votes?slug=eq.${encodeURIComponent(slug)}&select=id`
  ...
  async function loadReviews() {
    const url = `${api}/rest/v1/performer_reviews`
  ...
  function renderReviews(rows) {
    if (!rows.length) {
      ...
      return '<p class="note">この方の口コミは、まだ1件もありません。'
        + '出演作品を見た感想を、最初の一言として書いていただけます。</p>'
    }
  ...
  const form = `
    <form id="reviewForm" class="review-form">
      ...
      <textarea id="reviewBody" rows="4" maxlength="400" required
                placeholder="例）この作品の演技がよかった、この作品から見はじめた"></textarea>
  ...
  async function vote(event) {
    ...
    const response = await fetch(`${api}/rest/v1/performer_votes`, {
      method: 'POST',
    ...
    if (response.ok || response.status === 409) {
      localStorage.setItem(`darekore-voted-${slug}`, '1')
  ...
  async function send(event) {
    ...
    const response = await fetch(`${api}/rest/v1/performer_reviews`, {
      method: 'POST',
      headers: { ...json, Prefer: 'return=minimal' },
