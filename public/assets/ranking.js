...
(function () {
  ...
  async function load() {
    const url = `${api}/rest/v1/performer_stats?select=slug,votes,reviews&order=votes.desc&limit=100`
  ...
  function render(rows) {
    ...
    return `<ol class="rank-list">${ranked.map((row) => `
      <li>
        <a href="/actress/${encodeURIComponent(row.slug)}/">${escape(decodeURIComponent(row.slug))}</a>
