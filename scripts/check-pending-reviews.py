...
def main() -> None:
    ...
    rows = fetch(f'{api.rstrip("/")}/rest/v1/performer_reviews?{query}', key)
    ...
    lines += [
        ...
        '',
        '```sql',
        f'select approve_review(id) from performer_reviews where id in ({ids});',
        '```',
        '',
        '## 公開しないとき',
        '',
        '```sql',
        f'select reject_review(id) from performer_reviews where id in ({ids});',
        '```',
        '',
        '処理したら、この Issue を閉じてください。',
        '',
        # **同じ顔ぶれかどうかを、あとで機械が見分けるための印。**
        # 開いたままの Issue があるとき、この印が本文かコメントに
        # 既にあれば何もしない。無ければコメントを足す。
        # これが無いと、Issue を閉じ忘れている間に来た口コミが
        # まるごと知らされないまま溜まる（#9 が16日間そうなっていた）。
